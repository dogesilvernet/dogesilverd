...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("dogesilverdsanity-dogesilverd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("dogesilverd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "DOGESILVERD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "DOGESILVERD-STDERR")
