...
import (
	...
	"github.com/dogesilvernet/dogesilverd/domain/consensus/utils/mining"
	...
	"github.com/dogesilvernet/dogesilverd/stability-tests/common"
	"github.com/dogesilvernet/dogesilverd/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("dogesilverd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("dogesilverd datadir: %s", dataDir)

	dogesilverdCmd, err := common.StartCmd("DOGESILVERD",
		"dogesilverd",
	...
	spawn("startNode-dogesilverdCmd.Wait", func() {
		err := dogesilverdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("dogesilverdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes dogesilverd closes ungracefully
				log.Errorf("dogesilverdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(dogesilverdCmd, "dogesilverdCmd")
