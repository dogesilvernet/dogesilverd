...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("dogesilverd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("dogesilverd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("DOGESILVERD-SYNCER",
		"dogesilver",
	...
	syncedCmd, err := common.StartCmd("DOGESILVERD-SYNCED",
		"dogesilver",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
