...
func TestEstimateComputeMassAfterSignatures(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		signedTxStep1Bytes, err := libdogesilverwallet.Sign(params, mnemonics[:1], unsignedTransactionBytes, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		signedTxStep2Bytes, err := libdogesilverwallet.Sign(params, mnemonics[1:2], signedTxStep1Bytes, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		extractedSignedTx, err := libdogesilverwallet.ExtractTransaction(signedTxStep2Bytes, false)
		if err != nil {
...
func testEstimateMassIncreaseForSignaturesSetUp(t *testing.T, consensusConfig *consensus.Config) (
	*serialization.PartiallySignedTransaction, []string, *dagconfig.Params, func(keepDataDir bool)) {
	...
	unsignedTransaction, err := libdogesilverwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
