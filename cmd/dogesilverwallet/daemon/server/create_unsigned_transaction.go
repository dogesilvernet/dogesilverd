...
// By having at least 10DAG in the change output we make sure that the storage mass charged for change is
// at most 1000 gram. Generally, if the payment is above 10DAG as well, the resulting storage mass will be
// in the order of magnitude of compute mass and wil not incur additional charges.
// Additionally, every transaction with send value > ~0.1 DAG should succeed (at most ~99K storage mass for payment
// output, thus overall lower than standard mass upper bound which is 100K gram)
const minChangeTarget = constants.SompiPerDogesilver * 10
...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case nil:
		...
		feeRate = estimate.Estimate.NormalBuckets[0].Feerate
		// Default to a bound of max 1 DAG as fee
		maxFee = constants.SompiPerDogesilver
	}
...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool, requestFeePolicy *pb.FeePolicy) ([][]byte, error) {
	if !s.isSynced() {
		return nil, errors.Errorf("wallet daemon is not synced yet with Starbase, %s", s.formatSyncStateReport())
	}
	...
	if changeSompi > 0 {
		payments = append(payments, &libdogesilverwallet.Payment{
			Address: changeAddress,
			Amount:  changeSompi,
		})
	}
	unsignedTransaction, err := libdogesilverwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feeRate float64, maxFee uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libdogesilverwallet.UTXO, totalReceived uint64, changeSompi uint64, err error) {
...
func (s *server) selectUTXOsWithPreselected(preSelectedUTXOs []*walletUTXO, allowUsed map[externalapi.DomainOutpoint]struct{}, spendAmount uint64, isSendAll bool, feeRate float64, maxFee uint64, fromAddresses []*walletAddress) (
	...
	iteration := func(utxo *walletUTXO, avoidPreselected bool) (bool, error) {
		...
		selectedUTXOs = append(selectedUTXOs, &libdogesilverwallet.UTXO{
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.SompiPerDogesilver, float64(totalValue)/constants.SompiPerDogesilver)
	}

	return selectedUTXOs, totalReceived, totalValue - totalSpend, nil
}

func (s *server) estimateFee(selectedUTXOs []*libdogesilverwallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libdogesilverwallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libdogesilverwallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libdogesilverwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	mockUTXO := &libdogesilverwallet.UTXO{
	...
	mockTx, err := libdogesilverwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, []*libdogesilverwallet.UTXO{mockUTXO})
	if err != nil {
		return 0, err
	}
	...
	mockTxWithoutUTXO, err := libdogesilverwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err
