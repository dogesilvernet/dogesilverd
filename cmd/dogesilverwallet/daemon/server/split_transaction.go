...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libdogesilverwallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libdogesilverwallet.UTXO{
	...
	return libdogesilverwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) transactionFeeRate(psTx *serialization.PartiallySignedTransaction) (float64, error) {
	...
	if totalIns < totalOuts {
		return 0, errors.Errorf("Starbase response transaction don't have enough funds to pay for the outputs")
	}
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by dogesilverwallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libdogesilverwallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libdogesilverwallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libdogesilverwallet.UTXO, requiredAmount uint64, feeRate float64) (
	additionalUTXOs []*libdogesilverwallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libdogesilverwallet.UTXO{
