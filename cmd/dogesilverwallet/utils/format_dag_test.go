...
// Takes in a string representation of the Dag value to convert to Sompi
func TestDagToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := DagToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := DagToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateDAGAmountFormat(testCase)
