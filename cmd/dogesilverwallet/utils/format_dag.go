...
// FormatDag takes the amount of sompis as uint64, and returns amount of DAG with 8  decimal places
func FormatDag(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerDogesilver)
	}
	return res
}

// DagToSompi takes in a string representation of the Dag value to convert to Sompi
func DagToSompi(amount string) (uint64, error) {
	err := validateDAGAmountFormat(amount)
	...
	if constants.SompiPerDogesilver%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerDogesilver is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerDogesilver))
	decimalStr := ""
...
func validateDAGAmountFormat(amount string) error {
