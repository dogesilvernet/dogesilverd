...
import (
	...
	"github.com/dogesilvernet/dogesilverd/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		dogesilverdMessage, err := client.Post(&protowire.DogesilverdMessage{Payload: &protowire.DogesilverdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := dogesilverdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	dogesilverdMessage := &protowire.DogesilverdMessage{}
	err := protojson.Unmarshal([]byte(response), dogesilverdMessage)
	if err != nil {
	...
	return marshalOptions.Format(dogesilverdMessage)
}
