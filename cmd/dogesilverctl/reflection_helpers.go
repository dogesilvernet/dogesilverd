...
// 2. A wrapper that implements isDogesilverdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `DogesilverdMessage_` prefix and `Request` suffix
...
// generateDogesilverdMessage generates a wrapped DogesilverdMessage with the given `commandValue`
func generateDogesilverdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.DogesilverdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	dogesilverdMessage := reflect.New(reflect.TypeOf(protowire.DogesilverdMessage{}))
	dogesilverdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return dogesilverdMessage.Interface().(*protowire.DogesilverdMessage), nil
}
