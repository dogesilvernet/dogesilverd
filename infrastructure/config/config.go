...
const (
	defaultConfigFilename      = "dogesilverd.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "dogesilverd.log"
	defaultErrLogFilename      = "dogesilverd_err.log"
	...
	sampleConfigFilename    = "sample-dogesilverd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for dogesilverd.
	DefaultAppDir = util.AppDir("dogesilverd", false)
...
//go:embed sample-dogesilverd.conf
...
// Flags defines the configuration options for dogesilverd.
//
...
// Config defines the configuration options for dogesilverd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default dogesilverd configuration
...
//
// The above results in dogesilverd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-dogesilverd.conf to the given destination path,
