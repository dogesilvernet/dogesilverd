...
// Outpoint defines a dogesilver data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new dogesilver transaction outpoint point with the
...
// TxIn defines a dogesilver transaction input.
type TxIn struct {
...
// NewTxIn returns a new dogesilver transaction input with the provided
...
// TxOut defines a dogesilver transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new dogesilver transaction output with the provided
...
// MsgTx implements the Message interface and represents a dogesilver tx message.
