...
import (
	...
	"github.com/dogesilvernet/dogesilverd/infrastructure/os/execenv"
	"github.com/dogesilvernet/dogesilverd/infrastructure/os/limits"
	"github.com/dogesilvernet/dogesilverd/infrastructure/os/signal"
	"github.com/dogesilvernet/dogesilverd/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "dogesilverdsvc",
	DisplayName: "dogesilverd Service",
	Description: "Downloads and stays synchronized with the dogesilver blockDAG and " +
		"provides DAG services to applications.",
}

type dogesilverdApp struct {
	cfg *config.Config
}

// StartApp starts the dogesilverd app, and blocks until it finishes running
func StartApp() error {
...
func (app *dogesilverdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start dogesilverd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down dogesilverd...")
		...
		log.Infof("dogesilverd shutdown complete")
	}()
