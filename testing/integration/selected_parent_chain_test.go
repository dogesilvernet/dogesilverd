...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of dogesilverd instances
	dogesilverd1, dogesilverd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := dogesilverd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In dogesilverd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, dogesilverd2)
	}

	// Connect the two dogesilverds. This should trigger sync
	// between the two nodes
	connect(t, dogesilverd1, dogesilverd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := dogesilverd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
